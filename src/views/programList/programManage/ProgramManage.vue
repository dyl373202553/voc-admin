            ...
            <div class="dtable">
                ...
                <el-table-column label="操作" align="center">
                    ...
                    <router-link v-if="scope.row.liveEntity.summaryFlag === '0' && !scope.row.summaryEntity && userrole===0"
                        :to="{name:'ProgramSummary', params: { summaryName:'发布小结', id:scope.row.id },query: { summaryName:'发布小结', id:scope.row.id } }">
                        <el-button type="text" size="small">发布小结</el-button>
                    </router-link>
                    <router-link v-if="scope.row.liveEntity.summaryFlag === '0' && scope.row.summaryEntity && userrole===0"
                        :to="{name:'ProgramSummary', params: { summaryName:'管理小结', id:scope.row.id },query: { summaryName:'管理小结', id:scope.row.id } }">
                        <el-button type="text" size="small">管理小结</el-button>
                    </router-link>
                    <router-link
                        :to="{name:'ProgramRelease', params: { summaryName:'detail', id:scope.row.id},query: { summaryName:'detail', id:scope.row.id } }">
